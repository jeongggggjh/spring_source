...
public class Sprweb14jpaApplication {
	...
	private void selectData() {
		...
		ProductVo vo = repository.findById(2).get();
			System.out.println(vo.getCode() + " " + 
						vo.getSang() + " " +
						vo.getSu() + " " +
						vo.getDan());
		...
		System.out.println();
		ProductVo vo3 = repository.findByCode(1); // 메소드 이름으로 쿼리 생성
		...
		ProductVo vo4 = repository.findByCodeMy(1); // 메소드 이름 임의 생성
		System.out.println(vo4.getCode() + " " + 
				vo4.getSang() + " " +
				vo4.getSu() + " " +
				vo4.getDan());
		
		ProductVo vo5 = repository.findByCodeMy2(2); // 메소드 이름 임의 생성(순서 기반)
		...
		System.out.println();
		List<ProductVo> list3 = repository.findByData(3, "파이");
